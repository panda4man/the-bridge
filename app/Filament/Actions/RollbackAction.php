 ...
 * 2. `!source.commit_sha` → 400. Kept, as a refusal with a danger
 *    notification. The action is also hidden for deployments that cannot be
 *    rolled back to, but hiding is presentation: the guard still runs, since
 *    a stale page could submit against a row whose SHA has since been
 *    cleared.
        ...
        return Action::make($name)
            ...
            ->action(static function (Deployment $record): mixed {
                if (! $record->commit_sha) {
                    Notification::make()
                        ->danger()
                        ->title('Deployment has no commit SHA — cannot rollback.')
                        ->send();
